...
<section>
    <x-panel icon="cube">
        @if (session('status'))
            <div class="mb-4 font-medium text-sm text-white flex flex-row items-center justify-center p-4 rounded-lg bg-[#219654]">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form action="{{ route('login') }}" method="POST">
            ...
            <div class="flex flex-row justify-end mt-10">
                <x-button href="{{ route('password.request') }}" target="_self" class="bg-transparent text-on-surface-500 px-0 py-0 hover:text-on-surface-600 hover:bg-transparent">
                    {{ __('Forget password') }}
                </x-button>
