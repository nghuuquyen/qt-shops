<div x-transition:enter="transition fade-in duration-300" x-transition:leave="transition fade-out duration-300"
    ...
    x-data class="bg-white py-4 mt-6 fixed bottom-0 left-0 right-0 px-2 lg:px-0" x-show="$wire.display">
        ...
        <div class="grow flex">
            ...
            <ul class="flex flex-wrap ml-2">
                @foreach ($cart->items as $item)
                    <li class="flex flex-row border rounded-lg px-2 py-1 mr-2 mt-2 cursor-pointer hover:bg-green-500 hover:text-white"
                        @click="Livewire.emit('USER_SELECT_PRODUCT_EVENT', {{ $item->product_id }})">
                        <span class="mr-2">{{ $item->quantity }}x {{ $item->product->name }}</span>
