<?php

namespace App\Policies;

use App\Models\ReportFile;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class ReportFilePolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return $user->can('viewAny reports');
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, ReportFile $reportFile): bool
    {
        return $user->can('view reports');
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return $user->can('create reports');
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, ReportFile $reportFile): bool
    {
        return $user->can('update reports');
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, ReportFile $reportFile): bool
    {
        return $user->can('delete reports');
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, ReportFile $reportFile): bool
    {
        return $user->can('delete reports');
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, ReportFile $reportFile): bool
    {
        return $user->can('delete reports');
    }
}
